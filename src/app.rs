...
impl MainApp {
    ...
    fn read_shortcuts(ctx: &Context) -> (Modifiers, ShortcutFlags) {
        ctx.input(|i| {
            let mut flags = ShortcutFlags::default();
            ...
            for event in &i.events {
                ...
                } = event
                {
                    ...
                    match key {
                        Key::V => flags.paste = true,
                        Key::C => flags.copy = true,
                        Key::X => flags.cut = true,
                        Key::A => flags.select = true,
                        Key::Z => {
                            if modifiers.shift {
                                flags.redo = true;
                            } else {
    ...
    fn handle_clipboard_shortcuts(&mut self, ctx: &Context, app: &AppHandle) {
        let (modifiers, flags) = Self::read_shortcuts(ctx);
        if !flags.any() {
            return;
        }

        let paste_text = flags
            .paste
            .then(|| app.clipboard().read_text().ok())
            .flatten();

        ctx.input_mut(|i| {
            i.events.retain(|event| match event {
                Event::Text(t) if flags.paste && (t == "v" || t == "V") => false,
                Event::Text(t) if flags.copy && (t == "c" || t == "C") => false,
                Event::Text(t) if flags.cut && (t == "x" || t == "X") => false,
                Event::Text(t) if flags.select && (t == "a" || t == "A") => false,
                Event::Text(t) if flags.undo && (t == "z" || t == "Z") => false,
                Event::Text(t) if flags.redo && (t == "y" || t == "Y") => false,
                _ => true,
            });

            if let Some(text) = paste_text.filter(|text| !text.is_empty()) {
                i.events.push(Event::Paste(text));
            }

            if flags.copy {
                i.events.push(Event::Copy);
            }

            if flags.cut {
                i.events.push(Event::Cut);
            }
    ...
    fn handle_clipboard_output(ctx: &Context, app: &AppHandle) {
        for command in ctx.output(|o| o.commands.clone()) {
    ...
    fn update(&mut self, ctx: &Context, app: &AppHandle) {
        ...
        self.handle_clipboard_shortcuts(ctx, app);
        ...
        Self::handle_clipboard_output(ctx, app);
    }
}
...
impl ShortcutFlags {
    fn any(&self) -> bool {
        self.paste || self.copy || self.cut || self.select || self.undo || self.redo
    }
}
